...
class BitField(object):
    ...
    def extract(self, word: int) -> int:
        ...
        """
        word = (word >> self.from_bit) & self.mask
        return word
    ...
    def insert(self, value: int, word: int) -> int:
        """Insert value, which should be in the low order
         ...
         """
        value = (value & self.mask << self.from_bit)
        return word | value

    def extract_signed(self, word: int) -> int:
        """Extract bits in bitfield as a signed integer."""
        word = (word >> self.from_bit) & self.mask
        return sign_extend(word, self.to_bit - self.from_bit + 1)
...
def sign_extend(field: int, width: int) -> int:
    ...
    if (field & sign_bit):
        ...
        #return word|value
        #word = word & self.mask
        #extract = ((word & self.mask) >> self.from_bit) | value