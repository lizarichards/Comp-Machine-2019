...
class Instruction(object):
    ...
    def encode(self) -> int:
        ...
        result = 0
        result = reserved.insert(0, result)
...
def decode(word: int) -> Instruction:
    ...
    opcode = OpCode(instr_field.extract(word))
    cond = CondFlag(cond_field.extract(word))
    target = reg_target_field.extract(word)
    src_1 = reg_src1_field.extract(word)
    src_2 = reg_src2_field.extract(word)
    result = Instruction(opcode, cond, target, src_1, src_2, offset)
    return result
