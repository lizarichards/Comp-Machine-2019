"""
Name: Connor Finch
Project: Assembler Phase I for DM2019W assembly language.
Description:
...
class AsmSrcKind(Enum):
    ...
    DATA = auto()
    # An instruction that refers to a memory location in place of its source and offset parts
    MEMOP = auto()
    # An instruction that jumps to a certain line by using the program counter
    JUMP = auto()
...
ASM_MEMOP_PAT = re.compile(r"""
   ...
   \s*
    ...
    \s+
    (?P<target>    r[0-9]+),            # Target register
    (?P<labelref>  [a-zA-Z]\w*)         # Reference label 
   # Optional comment follows # or ; 
   (
     \s*
     (?P<comment>[\#;].*)
    ...
    for lnum in range(len(lines)):
        ...
        try: 
            ...
            if fields["kind"] == AsmSrcKind.FULL:
                ...
                mem_addr = table[fields["labelref"]]
                ...
                fix_optional_fields(fields)
                f = fields
                full = (f"{f['label']}   {OpCode.ADD.name}{f['predicate']} " +
                        f" r15,r0,r15[{pc_relative}] #{f['labelref']} " +
                        f" {f['comment']}")
                transformed.append(full)
            else:
                log.debug("Valid ASM pattern not found")
                transformed.append(line)
...
def fix_optional_fields(fields: Dict[str, str]):
    ...
    they require.
    """
    fields["label"] = "" if fields["label"] is None else fields["label"] + ":"
    fields["predicate"] = "" if fields["predicate"] is None else "/" + fields["predicate"]
